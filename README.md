# Netherite Compass

![Java 25](https://img.shields.io/badge/language-Java%2025-9b599a.svg?style=flat-square&color=red&logo=data:image/png;base64,)
[![Project License](https://img.shields.io/github/license/DorkixAzIgazi/NetheriteCompass?style=flat-square)](LICENSE)
![Environment: Client and Server](https://img.shields.io/badge/environment-client_and_server-1976d2?style=flat-square)
![Mod loader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)

![Version](https://img.shields.io/badge/minecraft_version-26.3-3b8526?style=flat-square&logo=data:image/png;base64,)
![Mod](https://img.shields.io/badge/mod_version-1.17.0-3b8526?style=flat-square)

![Modrinth](https://img.shields.io/badge/modrinth-Netherite_Compass-1bd96a?style=flat-square&color=1bd96a&link=https%3A%2F%2Fmodrinth.com%2Fmod%2Fnetherite-compass&logo=data:image/svg+xml;base64,)
![Curseforge](https://img.shields.io/badge/curseforge-Netherite_Compass-f16536?style=flat-square&color=f16536&link=https%3A%2F%2Fwww.curseforge.com%2Fminecraft%2Fmc-mods%2Fnetherite-compass&logo=data:image/png;base64,)

## About the mod

This mod adds the Netherite Compass to your game which helps you track down Ancient Debris blocks in you world.

![Netherite Compass](/docs/images/compass.gif)

### Behaviour:

#### Tracking:

The Netherite Compass will point to the nearest Ancient Debris in you 3x3 chunk area. If it finds an Ancient Debris block it will lock onto it and will point to it (even if you move and another Ancient Debris is closer) until the block is broken.

![Found Debris](/docs/images/found.png)

#### Locating:

If the compass didn't managed to find Ancient Debris in your nearby are it will start spinning. This means its continuously looking for them while you have it on you.

![Found Debris](/docs/images/locating.png)

#### Changing Dimensions:

If you go to another dimension and your compass is locked onto an Ancient Debris, it will warn you in its description that the Ancient Debris is in another dimension. It will start wobbling around like the default compass when it can't find its target.

![Wrong Dimension](docs/images/other_dim.png)

#### Using the compass:

When you right click the compass it will try to search for a new target if it already had one. This action will give you a five second cooldown before you can use it again (just like the ender pearl when thrown).

#### Dropped and Item frames

The compass **will not** change its state if its dropped or in an Item frame. For example, if its in the "Locating..." state and you put into an Item frame it **will not** lock onto an Ancient Debris if you place one nearby, like it would if you had it in your inventory.

### Crafting

Old crafting recipe used to require a lot of netherite, making the item useless as the player could have already crafted all the netherite gear and tools from the required amount of netherite. This new recipe encourages exploration and gives a fair reward for conquering two harder structures.

In order to obtain the Netherite Compass the player have to put three items into the **Smithing Table**:

- **Netherite Upgrade Smithing Template**
- **Recovery Compass**
- **Netherite Ingot**

![Crafting](/docs/images/crafting.png)

### Configuring search radius with Modmenu

This mod integrates with [Mod Menu](https://modrinth.com/mod/modmenu) to provide an in-game configuration screen. You can adjust the chunk search radius to balance between performance and search range.

To access the configuration:

1. Install Mod Menu (if not already installed)
2. Open the Mods menu from the main menu or pause screen
3. Find "Netherite Compass" in the mod list
4. Click the config button (options icon) to open the configuration screen
5. Adjust the chunk radius slider (1-16 chunks)
6. Click "Save" to apply your changes

**Note:** Larger search radii will scan more chunks, which may impact performance on slower systems. The default value of radius 1 -> 3x3 chunks should be more than enough for vanilla worlds.

![Mod Menu Config](/docs/images/modmenu.png)

### Other notes

The compass will not show height offcourse, just like the others already in game, but it is already powerful enough so it probably don't need to.

### Loading worlds without the mod

If you **load up a world without the mod**, which you previously used it with, only the compass will disappear, the **Ancient Debris Blocks will not be affected, they won't disappear from your world**.

## Building from source

#### Prerequisites

- Java 25 or above

#### Compiling

Navigate to the directory you've cloned this repository and launch a build with Gradle using `gradlew build` (Windows)
or `./gradlew build` (macOS/Linux). If you are not using the Gradle wrapper, simply replace `gradlew` with `gradle`
or the path to it.

The initial setup may take a few minutes. After Gradle has finished building everything, you can find the resulting
artifacts in `build/libs`.

## Why is Forge no longer supported?

This mod is developed exclusively for **Fabric** and does not support Forge or NeoForge. Here's why:

### Maintenance Burden

- Maintaining separate codebases for Fabric and Forge significantly increases development time and complexity
- The two mod loaders have fundamentally different APIs, requiring essentially two separate implementations
- Bug fixes and new features would need to be implemented twice, tested twice, and debugged twice

### Technical Incompatibility

- Fabric and Forge use completely different architectures and mixin systems
- The mod relies heavily on Fabric-specific APIs and features that don't have direct Forge equivalents
- Compatibility layers (like Sinytra Connector) are experimental and unreliable for production use

### Development Focus

- By focusing on a single mod loader, I can dedicate more time to adding features and improving the mod
- The Fabric ecosystem provides better tooling and a more modern development experience
- The Fabric community and documentation make it easier to maintain high-quality code

If you're looking for a Forge version, you're welcome to fork this project and create a port yourself (The MIT license allows you to do so freely), or consider switching to Fabric for this mod.

## License

This mod is licensed under MIT, a free and open-source license. For more information, please see the
[license file](LICENSE).
